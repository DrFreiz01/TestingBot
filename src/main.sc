...
theme: /
    ...
    state: Начало
        ...
        script:
            $session.FirstStateCount = $session.FirstStateCount || 0; ++$session.FirstStateCount;   // считает количество попаданий на первый стейт (пользовательская переменная)
            updateClientInfo();
        if: !((($client || {}).user || {}).name)
            a: Привет! Я виртуальный помощник сотрудников Hoff.
        else:
            a: Привет, {{ $client.user.name }}! Я виртуальный помощник сотрудников Hoff.
        a:  Могу подсказать:
            <ul>
...
#                <li><a href="http://confluence.kifr-ru.local:8090/pages/viewpage.action?pageId=46041071" target="_blank">Все навыки</a>.</li>
    ...
    state: helpdesk || noContext=true
        ...
        a:  В службе технической поддержки Hoff (helpdesk):
            ...
            И ещё, последнее по порядку, но не по значению:
            <ul>
...
#            Ожидайте, пожалуйста...
        ...
        state: Switch || noContext=true
            script:
                $session.OperatorTransCount = $session.OperatorTransCount || 0; ++$session.OperatorTransCount;      // считает количество попыток переключения на оператора (пользовательская переменная)
                ...
                || $session.NoMatchCount > 1) {
                    smartSwitch($session.smartSwitchLastState);                                                     // запуск функции переключения на оператора
                } else {
...
#            #Если бот не знает что ответить, то он предлагает переформулировать вопрос
#            if: ($session.NoMatchCount == 1 && !$session.OperatorTransFlag)
#                a:  На ваше сообщение я не нахожу ответа, пожалуйста, попробуйте переформулировать его в краткой форме.
#                    - если я не найду подходящего ответа, тогда - переколючу на оператора.
#            #Если это первая попытка переключения, то бот предлагает переформулировать вопрос
#            if: ($session.FirstStateCount == 1 && !$session.OperatorTransFlag)
#                a:  Моя задача - помочь вам без необходимости подключения оператора.
#                    - операторы обычно заняты решением сложных вопросов и лучше их не отвлекать без особой необходимости.
#                    - если мне не получиться вам помочь, тогда уже я попробую переключить вас на оператора.
#                    Переформулируйте, пожалуйста, ваш вопрос в краткой форме.
#            #Если количество попыток переключения удовлетворяет нормативу, то бот разрешает переключить (то есть выставляет флаг $session.OperatorTransFlag == true
#            if: ($session.FirstStateCount > 1 && !$session.OperatorTransFlag)
#                script: 
#                    $session.OperatorTransFlag = true;
#            #Переключение на оператора (в т.ч. и при любой ошибке), т.е. $session.OperatorTransFlag == true
#            if: ($session.OperatorTransFlag)
#                script:
...
#                });
            ...
            state: livechatFinished || noContext=true
                ...
                if: $session.smartSwitchCheck == true;
                    script:
                        $session.smartSwitchCheck = false;
                    a:  — Оператор отключен —
                        <ul>
            ...
            state: NoOperatorsOnline || noContext=true
                ...
                script:
                    ...
                    &&  $session.smartSwitchlastGroup != "all")
                        ...
                        {
                            $reactions.answer('— Нет доступных операторов —<ul><li>перевожу обратно на виртуального помощника.</li></ul><br>— С вами снова виртуальный помощник —<ul><li>график работы операторов с 10 до 19 (МСК) ежедневно;</li><li>напишите иной вопрос или попробуйте позже.</li></ul>');
