theme: /documentsFaq
    ...
    state: howToAddDocument
        ...
        q!: * ($how/$want/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * ваш* ~почта * ($ticket/$airplaneTicket/$flight/~заказ/~зоказ/~поездка/~рейс) * $return *
        q!: * ($how/$want/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * ваш* ~почта * $return * ($ticket/$airplaneTicket/$flight/~заказ/~зоказ/~поездка/~рейс) *
