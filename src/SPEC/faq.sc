theme: /faq
    ...
    state: howToPlaceOrder
        ...
        q!: ($how/$want/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic [ли]) [$meDat] {$issueVerbInf (заказ*/зоказ*)} [$markQuestion]
        q!: {$issueVerbInf (заказ*/зоказ*)} [$meDat] ($how/$want/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic [ли]) [$markQuestion]
        script:
            $client.topic = "Вопросы по оформлению заказа";
    ...
    state: registerAsLegalEntity
        ...
        q!: * {(заключит*/заключен*/хаключен*/подписат*/падписат*/подписани*) ~договор} * (оказан* услуг*/{($issueNoun/$buyNoun/~заказ/~зоказ/~оплата/~плата/~уплата/бронировани*/~бронь) ($ticket/$airplaneTicket/$railwayTicket/$flight/авиа [и] $railwayTicket)}) *
        q!: * (оказан* услуг*/{($issueNoun/$buyNoun/~заказ/~зоказ/~оплата/~плата/~уплата/бронировани*/~бронь) ($ticket/$airplaneTicket/$railwayTicket/$flight/авиа [и] $railwayTicket)}) * {(заключит*/заключен*/хаключен*/подписат*/падписат*/подписани*) ~договор} *
    ...
    state: buyTicketsAsLegalEntity
        ...
        q!: * ($how/$want/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * {$issueAllExceptPast ~заказ} * ($ticket/$airplaneTicket/$railwayTicket/$flight/авиа [и] $railwayTicket) * $payAll * (организац*/компани*/$legalEntity/$ip) [$markQuestion]
        q!: * ($how/$want/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * ($ticket/$airplaneTicket/$railwayTicket/$flight/авиа [и] $railwayTicket) * {$issueAllExceptPast ~заказ} * $payAll * (организац*/компани*/$legalEntity/$ip) [$markQuestion]
    ...
    state: error
        ...
        q!: * {($succeedInNot/$cannot2/($ne (выходит/выходет/дает/даёт/выходит/выходет)/недает/недаёт/невыходит)) [$meDat] ($buyAllExceptPast/$buyVerbPast/$issueVerb/$bookInf/заказ*/$registerInf)} * $weight<+1>
...
#        a: В связи c закрытием авиасообщения между Россией и Грузией с 8 июля 2019 года, российские авиакомпании разрешили вынужденный возврат ранее купленных билетов. Оформить возврат вы можете в личном кабинете на сайте Ozon.Travel. \nДля этого зайдите в ваш личный кабинет и напротив номера заказа выберите «Заявка на возврат», а в причине возврата укажите «вынужденный». Далее выберите признак «отмена рейса». \nБолее подробную информацию можно найти по ссылке: https://www.ozon.travel/help/avia/flights-to-Georgia
    ...
    state: orderNumber
        ...
        q!: * {(~заказ/~зоказ) ($ne (вижу/видн*/виден/видны/отображ*/отоброж*))} * $orderNumber *
        q!: * $orderNumber * {(~заказ/~зоказ) ($ne (вижу/видн*/виден/видны/отображ*/отоброж*))} *
        go!: /operator/goToOpeartor
    ...
    state: flightStatus
        ...
        q!: * {($annulNoun/$return/отмена/отмены/сдача) ([$my2/$ruSvoi] ($ticket/$airplaneTicket/$flight/~заказ/~зоказ/~поездка/~рейс))} * (закрыл*/закроют*/отмен*) * ($flight/~рейс) * $countryName *
        q!: * (закрыл*/закроют*/отмен*) * ($flight/~рейс) * $countryName * {($annulNoun/$return/отмена/отмены/сдача) ([$my2/$ruSvoi] ($ticket/$airplaneTicket/$flight/~заказ/~зоказ/~поездка/~рейс))} *
