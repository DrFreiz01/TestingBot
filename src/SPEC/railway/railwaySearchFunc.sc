theme: /railwaySearchFunc
    ...
    state: varsCleaning_DYN
        script:
            ...
            $session.httpResponse1 = undefined;
            $session.httpResponse2 = undefined;
            ...
            if ($session.dates1 && $session.dates2 && !$session.dates) {
                ...
                if ($session.from && $session.dest && $session.from != $session.dest) {
                    ...
                    if (isArrayEmpty(httpResponse1) && isArrayEmpty(httpResponse2)) {
                        ...
                        var dateTime = $session.httpResponse[tripOfferNumb].segments[0].departureTime.time.split('T');
                        var fromDate = dateTime[0];
                        var dates4User1 = getDate4User(fromDate);
                        $session.stationF = $session.httpResponse[tripOfferNumb].segments[0].departureStation.name;
                        $session.stationT = $session.httpResponse[tripOfferNumb].segments[0].arrivalStation.name;
                        var category = $session.httpResponse[tripOfferNumb].tariffs[tariffNumb].category;
                ...
                if ($session.from && $session.dest && $session.dates && $session.from4User &&  $session.dest4User && ($session.from != $session.dest)) {
                    //проверяем, что даты не в прошлом
                    if (datesComparison($session.dates) == false) {
                        ...
                        if (result.data && result.data.trips[0]) {
                            $session.httpResponse = result.data.trips[0].tripOffers;
                        } else {
                        ...
                        if (result.isOk && (!isArrayEmpty($session.httpResponse)) && result.status >= 200 && result.status < 300) {
                            var tripOfferNumb = getRailwayCategory($session.httpResponse)[0];
                            var tariffNumb = getRailwayCategory($session.httpResponse)[1];
                            var dateTime = $session.httpResponse[tripOfferNumb].segments[0].departureTime.time.split('T');
                            var fromDate = dateTime[0];
                            ...
                            var category = $session.httpResponse[tripOfferNumb].tariffs[tariffNumb].category;
                            $session.category = getCategory4User(category);
                            $session.railwayPrice = $session.httpResponse[tripOfferNumb].tariffs[tariffNumb].minPrice;
                            //формируем ссылку на OZON
                            $session.urlRailway = urlOzonRailway({from: $session.from, dest: $session.dest, date: $session.dates, passengers: $session.adults});
        ...
        script:
            ...
            if ($session.dates1 && $session.dates2 && ($session.from != $session.dest)) {
                ...
                if ((datesComparison($session.dates1) == false) || (datesComparison($session.dates2) == false)) {
                    ...
                    var result1 = railwaySearchPost({from: $session.from, dest: $session.dest, date: $session.dates1, passengers: $session.adults});
                    if (result1.data && result1.data.trips[0]) {
                        $session.httpResponse1 = result1.data.trips[0].tripOffers;
                    } else {
                        $reactions.transition("/railwaySearchAns/searchError");
                    }
                    //проверяем, что есть ответ
                    if (result1.isOk && (!isArrayEmpty($session.httpResponse1)) && result1.status >= 200 && result1.status < 300) {
                        var dateTime = $session.httpResponse1[0].segments[0].departureTime.time.split('T');
                        var fromDate = dateTime[0];
                        var fromTime = dateTime[1];
                        $session.dates4User1 = getDate4User(fromDate);
                        $session.fromTime4User1 = getTime4User(fromTime);
                        $session.stationF1 = $session.httpResponse1[0].segments[0].departureStation.name;
                        $session.stationT1 = $session.httpResponse1[0].segments[0].arrivalStation.name;
                        $session.destTime1 = $session.httpResponse1[0].segments[0].arrivalTime.time;
                        //ищем цены
                        var arrPrices1 = [];
                        for (var i = 0; i < $session.httpResponse1.length; i++) {
                            var everyTrip = $session.httpResponse1[i];
                    ...
                    if (result2.data && result2.data.trips[0]) {
                        $session.httpResponse2 = result2.data.trips[0].tripOffers;
                    } else {
                        $reactions.transition("/railwaySearchAns/searchError");
                    }

                    //проверяем, что есть ответ
                    if (result2.isOk && (!isArrayEmpty($session.httpResponse2)) && result2.status >= 200 && result2.status < 300) {
                        var dateTime = $session.httpResponse2[0].segments[0].departureTime.time.split('T');
                        var fromDate = dateTime[0];
                        var fromTime = dateTime[1];
                        $session.dates4User2 = getDate4User(fromDate);
                        $session.fromTime4User2 = getTime4User(fromTime);
                        $session.stationF2 = $session.httpResponse2[0].segments[0].departureStation.name;
                        $session.stationT2 = $session.httpResponse2[0].segments[0].arrivalStation.name;
                        $session.destTime2 = $session.httpResponse2[0].segments[0].arrivalTime.time;

                        //ищем цены
                        var arrPrices2 = [];
                        for (var i = 0; i < $session.httpResponse2.length; i++) {
                            var everyTrip = $session.httpResponse2[i];
                            for (var k = 0; k < everyTrip.tariffs.length; k++) {
                                var everyTariff = everyTrip.tariffs[k];
                                for (var tariff in everyTariff) {
                                    if (tariff === 'minPrice') {
