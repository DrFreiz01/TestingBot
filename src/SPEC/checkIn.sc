theme: /checkIn
    ...
    state: planeSeat
        ...
        q!: * (выбор*/$chooseInf) (место/места/мест/сидение/сидения/сидений/кресло/кресла/кресел) [на/по] $order *
        q!: [на/по] $order * (выбор*/$chooseInf) (место/места/мест/сидение/сидения/сидений/кресло/кресла/кресел) [$markQuestion]
    ...
    state: autoCheckIn
        ...
        q!: * ($how/$want/$where/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * ($activate/$plugging/найти/включить/воспользоват*/пользовать*/пользоватся) {[$my2/$ruSvoi/наш*] ($airplaneTicketAll/$order)} [на/$about/$for/по/в/во/к] [онлайн/анлайн/он лайн] (~регистрация/регестрац*/ригистрац*/ригестрац*) [$markQuestion]
        ...
        q!: * ($how/$want/$where/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * ($issueVerbInf/$activate/$plugging/найти/пройти/проходит*/провести/совершить/сделать/делать/осуществить/включить/воспользоват*/пользовать*/пользоватся) (онлайн/анлайн/он лайн) (~регистрация/регестрац*/ригистрац*/ригестрац*) [на/$about/$for/по/в/во/к] [$my2/$ruSvoi/наш*] [$airplaneTicketAll/$order] [$markQuestion]
        # 4EX: Как воспользоваться бесплатной услугой Регистрация на рейс?
        q!: * ($how/$want/$where/$need2/$canSg1 ([ли] (я/йа)/ли [я/йа]) / (я/йа) $canSg1/$possiblePredic) * ($issueVerbInf/$activate/$plugging/найти/пройти/проходит*/провести/совершить/сделать/делать/осуществить/включить/воспользоват*/пользовать*/пользоватся) бесплатн* услуг* (~регистрация/регестрац*/ригистрац*/ригестрац*) [на/$about/$for/по/в/во/к] [$my2/$ruSvoi/наш*] [$airplaneTicketAll/$order] [$markQuestion]
    ...
    state: IsThereAutoCheckIn
        ...
        q!: [$beginningWords] {([$exist/доступн*] [ли]) {[бесплатн*] (авторегистр*/авторегестр*/авторигестр*/автомат* (~регистрация/регестрац*/ригистрац*/ригестрац*))} } [на/$about/$for/по/в/во/к] {[$my2/$ruSvoi/наш*] ($airplaneTicketAll/$order)} [$markQuestion]
        # 4EX: Добрый день! Подскажите, как воспользоваться услугой Бесплатная регистрация на рейс? И доступна ли она к нашим заказам?
        q!: * {[услуг*] (~регистрация/регестрац*/ригистрац*/ригестрац*) [бесплатн*]} * ($exist/доступн*) ли [она/они] [на/$about/$for/по/в/во/к] {[$my2/$ruSvoi/наш*] ($airplaneTicketAll/$order)} [$markQuestion]
        q!: * {[услуг*] (~регистрация/регестрац*/ригистрац*/ригестрац*) [бесплатн*]} * ($exist/доступн*) [ли] (она/они) [на/$about/$for/по/в/во/к] {[$my2/$ruSvoi/наш*] ($airplaneTicketAll/$order)} [$markQuestion]
