...
var rules = {
    "_DATE_NUMERIC": /\d{1,2}[^\d]?\d{0,2}[^\d]?\d{0,4}/,
    ...
    "_DAYS_RANGE": /(\d{1,2}(\s|-|)(го|е|)\s?(по|до|-|\s)\s?\d{1,2}(\s|-|)(го|е|)|\d{1,2}-\d{1,2})\s?(числ[а|о]|)\s?((январ|феврал|март|апрел|ма|июн|июл|август|сентябр|октябр|ноябр|декабр)[a-я]{0,2})?/,
...
// проверяем, пересекается ли новая сущность с одной из уже выделенных
function intersects_slots(start_idx, end_idx, slots) {
    for (var index = 0; index < slots.length; index++) {
        var slot = slots[index];
        if ((start_idx <= slot[1]) && (end_idx >= slot[0])) {
            return true;
        }
    }
    return false;
...
// возвращаем найденные подстроки и все индексы начала и конца для каждой подстроки
function match_all(query, temp) {
    var found_substrs = [];

    var offset = 0;
    while (true) {
        var result = query.match(temp);
        if (result) {
            var substr = result[0]; // то, что нашлось
            var start_idx = offset + result.index;
            var end_idx = start_idx + substr.length - 1;
            found_substrs.push([substr, start_idx, end_idx]);
            query = query.substr(result.index + substr.length);
            offset += result.index + substr.length; // смещаемся 
        } else {
            break;
        }
    }
    ...
    return found_substrs;
}

function get_case(query, slots) {
    try {
        var result = [];
        ...
        Object.keys(rules).forEach(function(key) {
            ...
            var matched_results = match_all(query, rules[key]);
            if (matched_results) {
                for (var i = 0; i < matched_results.length; i++) {
                    var temp = matched_results[i][0];
                    // является ли он наибольшим 
                    if (temp.length > dest_temp.length) {
                        var temp_start_idx = matched_results[i][1];
                        var temp_end_idx = matched_results[i][2];
                        // если не пересекается с уже выделенными
                        if (!intersects_slots(temp_start_idx, temp_end_idx, slots)) {
                            dest_temp = temp;
                            matched_case = key;
        ...
        if (dest_temp.length === 0) {
            dest_temp = null;
        } 
        return [matched_case, dest_temp, start_idx, end_idx];
    } catch (e) {
        return null;
    }
}
...
// возвращаем дату для числового предсставления
function get_date_numeric(entity) {
    entity = remove_all_letters(entity);
    var currentDate = get_current_date();
    var base_splits = entity.split(/[^\d]/);
    var splits = [];
    for (var i = 0; i < base_splits.length; i++) {
        if (base_splits[i].length > 0) {
            splits.push(base_splits[i]);
        }
    }
    ...
    // если есть месяц, то извлекаем месяц
    if (splits.length > 1) {
        month = parseInt(splits[1], 10) - 1;
    }

    // если есть год, извлекаем год
    if (splits.length > 3) {
        year = splits[2];
        if (year.length != 4) {
            year = "20" + year;
...
function handle_request(query) {
    ...
    try {
        ...
        var all_results = [];
        var slots = [];
        // идем в цикле пока можем выделить сущности
        var count = 0;
        while (true) {
            ...
            var res = get_case(query, slots);
            var matched_case = res[0];
            var entity = res[1];
            var start_idx = res[2];
            var end_idx = res[3];

            // если не смогли выделить
            if (entity === null) {
                break;
            }

            // запоминает диапазон максимального выделенного слота
            slots.push([start_idx, end_idx]);
        
            var result = get_date(matched_case, entity, query);
